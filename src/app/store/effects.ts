...
export class MarshalsEffects {
  ...
  loadMarshals$ = createEffect(() => this.actions$.pipe(
      ofType(ListActions.load),
      exhaustMap(() => this.httpService.getData()
        .pipe(
...
export class BattlesEffects {
  ...
  loadBattles$ = createEffect(() => this.actions$.pipe(
    ofType(BattlesActions.load),
    exhaustMap(() => this.httpService.getAllBattles().pipe(
